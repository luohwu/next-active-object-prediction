...
def main():
    ...
    for epoch in range(current_epoch + 1, train_args['epochs'] + 1):
        ...
        # train_loss = val2(train_dataloader, model, criterion, epoch,write_val)
        # val_loss = val(val_dataloader, model, criterion, epoch, write_val)
        val_loss = val2(val_dataloader, model, criterion, epoch, write_val)
...
def val(val_dataloader, model, criterion, epoch, write_val):
    ...
    with torch.no_grad():
        for i, data in enumerate(val_dataloader):
            ...
            acc_array, f1_array = compute_metrics2(outputs_mask, mask_numpy)
...
def val2(val_dataloader, model, criterion, epoch, write_val):
    model.eval()
    total_val_loss = 0.
    targets_all, predictions_all = [], []
    ...
    with torch.no_grad():
        for i, data in enumerate(val_dataloader):
            ...
            predictions_all.append(
                outputs.data.max(1)[1].cpu().numpy())  # outputs.data.max(1)[1] of shape [Batch_size, height, width]
    ...
    val_loss_avg = total_val_loss / len(val_dataloader.dataset)
    del targets_all, predictions_all
    print(f'[epoch {epoch}], [val loss {val_loss_avg:5f}], [acc {acc_:5f}], '
